...
public class TaskListPanel extends Panel {
	...
	private final static Logger log = LoggerFactory.getLogger(TaskListPanel.class);

	public TaskListPanel(String id, final IModel<Task> taskModel) {
		super(id, taskModel);
		log.debug("<init>");
		...
		final DataView<Task> dataView = new DataView<Task>("sorting", provider) {
			...
			protected void populateItem(final Item<Task> item) {
				...
				item.add(new Label("prio", ""+task.prio));
				item.add(new Label("text", task.text));
				item.add(new Link<Task>("edit") {
					@Override
					public void onClick() {
						TaskListPage page = (TaskListPage) getPage();
						page.setDefaultModelObject(task);
